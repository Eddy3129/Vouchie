...
export const MiniappUserInfo = () => {
  const { context, isReady, isMiniApp, openProfile, composeCast } = useMiniapp();
  ...
  const handleCast = () => {
    composeCast({ text: "Hello from Scaffold-ETH 2 MiniApp!", embeds: ["https://scaffoldeth.io"] });
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4">
      ...
      <div className="overflow-x-auto">
        ...
        <div className="mt-6">
          <h2 className="text-xl font-bold text-center mt-4">Actions</h2>

          <div className="flex flex-col gap-4 items-center">
            <div className="flex flex-wrap gap-2 justify-center">
              <button className="btn btn-primary" onClick={handleQuickAuth}>
                Sign In (Quick Auth)
              </button>

              <button className="btn btn-secondary" onClick={handleCast}>
                Compose Cast
              </button>
            </div>

            {authToken && (
              <div className="w-full max-w-md bg-base-200 p-4 rounded-lg break-all">
                <p className="font-bold text-sm mb-2">Auth Token:</p>

                <p className="text-xs font-mono">{authToken}</p>
              </div>
            )}
          </div>
        </div>
