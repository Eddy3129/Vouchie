...
const TaskDetailModal = ({ isOpen, onClose, goal, onSubmit, onGiveUp, onExtend }: TaskDetailModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#FDFBF7] w-full max-w-md rounded-[32px] p-6 soft-shadow animate-in slide-in-from-bottom-10 duration-300 max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl chubby-text text-stone-800 flex items-center gap-2">{goal.title}</h3>
        ...
        <div className="flex gap-4 mb-6">
          <div className="flex-1 bg-white p-3 rounded-2xl border border-stone-100">
            ...
            <p className="text-xl font-bold text-stone-700 flex items-center gap-1">
              <Wallet size={18} className="text-stone-400" /> ${goal.stake}
            </p>
          </div>
          <div className="flex-1 bg-white p-3 rounded-2xl border border-stone-100">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-4 rounded-2xl border border-stone-100">
            ...
            {!isSolo && (
              <textarea
                rows={2}
                placeholder="Tell your squad you did it..."
          ...
          <div className="bg-yellow-50 p-4 rounded-2xl border border-yellow-100">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="bg-yellow-200 text-yellow-800 text-xs font-bold px-2 py-1 rounded-full">PREMIUM</span>
            </div>
            <p className="text-xs text-yellow-700 mb-3 font-semibold">Extend deadline by 12 hours for a small fee.</p>
            <button
              onClick={() => onExtend(goal.id)}
              className="w-full py-2 bg-yellow-400 text-white rounded-xl font-bold hover:bg-yellow-500 transition-colors flex items-center justify-center gap-2"
            >
          ...
          <button
            ...
            className="w-full py-3 text-red-400 font-bold hover:bg-red-50 rounded-xl transition-colors flex items-center justify-center gap-2"
          >
