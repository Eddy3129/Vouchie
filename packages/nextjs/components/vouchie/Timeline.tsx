...
const Timeline = () => {
  ...
  return (
    <div className="flex flex-col w-12 items-center relative py-2 mr-4 flex-shrink-0">
      ...
      {times.map((time, i) => (
        <div key={i} className="relative z-10 bg-[#FDFBF7] py-1 mb-[35px] last:mb-0">
          <span className="text-[10px] font-bold text-stone-400">{time}</span>
        </div>
      ))}
      <div className="absolute w-16 h-0.5 bg-red-400 z-20 flex items-center" style={{ top: currentTimeTop }}>
        <div className="w-2 h-2 rounded-full bg-red-400 -ml-1" />
        <span className="absolute left-3 text-[9px] font-bold text-red-400 bg-red-50 px-1 rounded-sm">NOW</span>
      </div>
    </div>
  );
};
